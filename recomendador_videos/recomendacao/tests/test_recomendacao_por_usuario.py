...
logging.basicConfig(
    filename="test_recomendacao_user_based.log",
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
    encoding="utf-8"
)

class RecomendacaoUserBasedTestCase(TestCase):
    def setUp(self):
    ...
    def test_recomendacao_videos_pearson(self):
        """Testa a recomendação de vídeos utilizando similaridade de Pearson."""
        resultado = recomendar_videos_user_based(self.user1, metodo_similaridade="pearson", top_n=3)

        # Log dos resultados
        logging.info(f"Teste Recomendação Pearson - Usuário {self.user1.id}: {len(resultado)} vídeos recomendados.")
    ...
    def test_recomendacao_usuario_sem_historico(self):
        ...
        resultado = recomendar_videos_user_based(user_novo, metodo_similaridade="pearson", top_n=3)
