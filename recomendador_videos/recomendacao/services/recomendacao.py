...
def recomendar_videos_user_based(usuario, metodo_similaridade=calcular_similaridade_usuarios):
    ...
    for usuario_similar, score in similaridade_usuarios.items():
        if score > 0:
            videos_similares = VideoRating.objects.filter(user=usuario_similar, rating=1).select_related('video')
            for rating in videos_similares:
                if not VideoRating.objects.filter(user=usuario, video=rating.video).exists():
...
def recomendar_videos_itens_based(usuario):
    ...
    """
    videos_recentes = VideoRating.objects.filter(user=usuario, rating=1).order_by('-updated_at')[:5]
...
def recomendar_videos_hibrido_fusao(usuario, similaridade='pearson'):
    ...
    return combinar_recomendacoes(recomendacao_user_based, recomendacao_itens_based)
...
def recomendar_videos_hibrido_cascata(user, similaridade=calcular_correlacao_pearson):
    ...
    historico_videos = VideoRating.objects.filter(user=user, rating=1).order_by('-updated_at')[:5]
